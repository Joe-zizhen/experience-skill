---
name: 5w-ledger-v1-2
description: "Evidence-based Five Whys root cause analysis with an observable evidence-admission ledger. Use when the user explicitly requests an auditable evidence ledger before the final RCA, or for high-severity incidents, failures spanning a time window or multiple interacting causes, audit and postmortem reviews, handoffs to another agent or team, and closed-evidence case analysis where every material causal claim must be recorded as ADMIT, REJECT, or UNKNOWN. Do not use as the default 5W workflow, for routine localized bug fixes, or for general explanations. For code-related problems, inspect relevant code, configuration, tests, logs, and runtime evidence before naming a root cause."
---
...
- The user explicitly asks for an evidence-admission ledger or an auditable RCA.
...
## Evidence Acquisition (Live And Code Investigations)
...
Record what you inspected (paths, commands, log excerpts, timestamps) so each ledger row can cite it. With a supplied closed narrative instead, quote the case text as the supporting fact and apply the closed-evidence rules in Workflow step 2 and the precedence gate in step 6.
...
2. For code-related problems, inspect the relevant code, configuration, tests, logs, and runtime evidence before proposing any cause. **For failures spanning time windows, collect the timeline: deployment times, config changes, traffic shifts, first-error timestamps, and recovery events. Verify that each causal link's cause precedes its effect in the collected timeline.** When the evidence base is a supplied closed narrative (no live systems or artifacts accessible), facts stated in the case are admitted as stated and carry narrative-grade confidence (no higher than `probable`), and every material link that relies solely on the narrative — without independent artifact evidence — is capped at `probable`, with the missing artifacts named as verification needs. `confirmed` is reserved for links supported by direct observation, reproduction, or strong converging evidence.
...
4. Build an explicit Why chain. Each answer must causally explain the preceding effect and state the cause, mechanism, evidence, and confidence; do not jump to a preferred theory. When independent or converging causes are necessary, branch into multiple Why chains instead of forcing one linear story. Keep the evidence scope explicit for every chain: a cause confirmed for one cohort, time window, code path, or failure mode does not automatically explain the remaining cases.
...
6. Test material causal links and the strongest plausible rival explanation. Require temporal order and a credible mechanism; when practical, use reproduction, comparison, change correlation, or a counterfactual. Treat correlation alone as insufficient. For closed evidence, apply this precedence gate before accepting any causal link, root-cause label, verification need, or final problem statement:
   - If a specific property or mechanism is not stated or favored by the given facts, replace it with `unknown`; calling it a hypothesis does not admit it.
   - If evidence does not establish a root cause for a branch, end with `root cause not established`; the output shape does not require every branch to have a root cause.
   ...
   **When multiple causal branches must intersect for the failure to occur, name the minimal sufficient causal set — the conjunction of conditions without which the failure would not have occurred — and identify which members of that set are within the defined control boundary.**
   Example: "Memory leak alone was insufficient; the spike in concurrent requests alone was insufficient. The minimal sufficient causal set is {memory leak, traffic spike}. Within the control boundary, the memory leak is the actionable root cause — the design defect requiring correction. The traffic spike is a boundary condition the system is expected to tolerate at some threshold."
   If independent branches each cause the failure in different cohorts, time windows, code paths, or failure modes, keep them as separate causal branches rather than forcing a single set. The anti-pattern this rule prevents: listing "traffic spike" and "memory leak" as parallel independent root causes of the same failure when neither alone was sufficient. The correct pattern: name the minimal set, distinguish within-boundary members from boundary-condition members.
7. Stop when the next Why would become speculation, blame, immaterial to the problem definition, outside the chosen analysis scope, or solution design instead of cause analysis. The smallest evidence-closed mechanism is sufficient; do not format an unknown boundary as another Why or root cause, and do not keep searching for a deeper or shared cause merely to complete a template. Do not stop solely because a fact is unchangeable: record it as a boundary condition and, if the system was expected to tolerate it, ask why the consequence remained possible.
8. Run one bounded blind-spot pass before finalizing. Check the strongest rival explanation, a missing causal branch, missing or biased evidence, and the reconciliation of stated quantities — if two or more stated numbers bear on the same causal link, verify they are arithmetically consistent. An inconsistency does not falsify the causal identity, but the reconciliation is marked UNKNOWN with the discriminating evidence named. A reconciliation achieved only by supplying an unstated premise (for example, assuming what a total includes or which population a figure covers) is not a reconciliation: treat the premise as an unverified assumption, mark the reconciliation UNKNOWN, and name the discriminating evidence. Treat separate cohorts, failure modes, and cases independently unless direct evidence establishes a shared causal mechanism; do not synthesize a cross-case theory unless the user explicitly requests one. If the pass reveals a more fundamental evidence-backed cause that changes the chain, revise it once. Stop when the next concern is speculative, low-impact, or unsupported by evidence.
9. Return the actual problem as a concise, solution-neutral engineering statement: "The real problem is: under X condition, A produced Z through mechanism M, violating invariant I." Copy the supporting chain's narrowest material scope and lowest confidence explicitly into the statement; do not rely on headings or earlier qualifiers to carry them forward. A `probable`, `hypothesis`, or `unknown` link must not become a confirmed or broader claim in the summary. State a distinct control gap separately when relevant.
10. If confidence is incomplete, label the cause as a hypothesis and state the evidence needed to confirm or falsify it.
11. **Characterize the problem structure** to constrain the solution space without prescribing a fix. Based solely on the causal evidence admitted in the ledger, identify which defect shapes are present — more than one may coexist in the same incident:
    - **Single-point defect**: the causal mechanism is a localized error or omission (e.g., incorrect value, missing check, wrong condition) whose effects propagate from that point.
    - **Structural absence**: the causal mechanism operates through a missing contract, abstraction, invariant enforcement, or ownership boundary; the defect is the absence of a mechanism that should exist.
    - **Boundary condition**: the causal mechanism involves an external or accreted constraint that the system was expected to tolerate but could not (e.g., vendor SLA breach, platform limitation); the system's defense against this boundary was insufficient or absent.
    When evidence is insufficient to distinguish between shapes, state **UNKNOWN** with the discriminating evidence needed. State the rationale for each identified shape in one sentence — what property of the causal mechanism determines it. If a defect sits at an intersection (for example, a missing check located exactly at a system boundary), assign each applicable shape with its own rationale rather than forcing a single label. Do not evaluate fix invasiveness, cost, priority, or implementation approach. This characterization bridges the RCA to first-principle design by naming the type of solution space, not the solution.
...
- Do not edit files, modify code, change configuration, or implement fixes while constructing the RCA. If the current request authorizes later stages, complete the RCA handoff before continuing; do not request duplicate approval.
- Do not turn the root cause into a proposed solution. A fix is not the answer; the answer is the real problem that must be solved.
- Do not treat "human error" as a root cause. Ask why the system allowed the error.
- Do not force exactly five whys. Use only as many as the evidence supports; chain length is never a target.
...
- Use `confirmed` only for direct observation, reproduction, or strong converging evidence; use `probable` when the evidence favors one mechanism over material rivals and the remaining uncertainty does not change its causal identity or scope; use `hypothesis` when missing evidence could change the causal identity or scope. If multiple labels appear applicable, use the lower-confidence label.
...
- A `hypothesis` label alone does not justify reporting an unranked hidden mechanism; preserve concrete exploration only under the live-investigation and explicit-exhaustive exceptions in step 6.
- Analyze multiple cases independently. Do not add cross-case analogies, shared-root theories, or general lessons unless the user explicitly asks for synthesis and the shared mechanism is directly evidenced.
- Do not force one root cause when the evidence requires multiple causal branches.
...
- Use first-principle thinking only to check whether the cause has been reduced to a basic enough fact; defer solution design to a separate step.
...
- **Timeline** (when relevant to establishing temporal order of causal links)
- Evidence inspected
- Why chain or causal tree, including mechanism, evidence, and confidence for each material link. **When branches intersect, name the minimal sufficient causal set and distinguish within-boundary members from boundary-condition members.**
...
- Decision: `ADMIT`, `REJECT`, or `UNKNOWN`
...
3. If a concrete link supplied by the case or user requires an unobserved premise, use `REJECT` for that concrete link and keep the unresolved mechanism `UNKNOWN`. Do not generate a specific missing premise merely to populate a `REJECT` row. A `hypothesis` label does not admit the missing premise.
   `REJECT` means that the concrete claim is not admissible from the available evidence as stated, not that it has been falsified. `UNKNOWN` marks the unresolved boundary of the current RCA. Neither decision precludes reassessment when the named discriminating evidence is obtained.
...
5. Name missing discriminating evidence without predicting the specific cause or result it will reveal.
6. Build the final RCA only from `ADMIT` rows. `UNKNOWN` may appear only as an explicitly unresolved boundary. No final root-cause label or problem statement may introduce a claim absent from the ledger or broaden its scope or confidence.
7. Do not propose remedies. Keep confirmed mechanisms confirmed; do not become conservative when direct observation, reproduction, or strong converging evidence closes the exact link.
...
explicitly unresolved UNKNOWN boundaries.
</final>
```
